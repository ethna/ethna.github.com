...
`Miniblog_Action_Hello`
  このクラスは、 **アクションクラス** です。Helloアクションの実装と、遷移先（ビュークラス）の決定をします。 ``prepare()`` メソッドや ``preform()`` メソッドにおける ``return`` は、ビュークラス名にあたります。作成したてのこのファイルでは ``perform()`` メソッドでは、文字列の ``'hellp'`` が返されることにより、次に `Hello ビュー`` が実行されることを表します。
